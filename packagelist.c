...
static int read_package_list(struct packagelist_data *pkgl_dat) {
	...
	long (*sys_read_new)(unsigned int, char __user *, size_t) = (void *)kallsyms_lookup_name("sys_read");
	long (*sys_lseek_new)(unsigned int, off_t, unsigned int) = (void *)kallsyms_lookup_name("sys_lseek");
	long (*sys_open_new)(const char __user *, int, umode_t) = (void *)kallsyms_lookup_name("sys_open");
	...
	fd = sys_open_new(kpackageslist_file, O_RDONLY, 0);
	if (fd < 0) {
	...
	while ((read_amount = sys_read_new(fd, pkgl_dat->read_buf,
		...
		if (additional_read > 0)
			sys_lseek_new(fd, -additional_read, SEEK_CUR);	
...
static int packagelist_reader(void *thread_data)
{
	...
	long (*sys_read_new)(unsigned int, char __user *, size_t) = (void *)kallsyms_lookup_name("sys_read");
	long (*sys_inotify_add_watch_new)(int, const char __user *, u32) = (void *)kallsyms_lookup_name("sys_inotify_add_watch");
	long (*sys_inotify_init_new)(void) = (void *)kallsyms_lookup_name("sys_inotify_init");

	allow_signal(SIGINT);

	nfd = sys_inotify_init_new();
	if (nfd < 0) {
	...
	while (!kthread_should_stop()) {
		...
		if (!active) {
			res = sys_inotify_add_watch_new(nfd, kpackageslist_file, IN_DELETE_SELF);
			if (res < 0) {
		...
		event_pos = 0;
		res = sys_read_new(nfd, pkgl_dat->event_buf, sizeof(pkgl_dat->event_buf));
...
void packagelist_destroy(void *pkgl_id)
{
	...
	int (*force_sig_info_new)(int, struct siginfo *, struct task_struct *) = (void *)kallsyms_lookup_name("force_sig_info");

	force_sig_info_new(SIGINT, SEND_SIG_PRIV, pkgl_dat->thread_id);
